...
The chosen model to represent the electricity distribution network consists of an undirected graph which nodes can be of three kinds:
...
2. **Connection nodes:** This nodes represent the direct interaction (connection) among device nodes. For example, to model a turbine connected to a generator the model should include a device node that represents each of them and a connection node that serves as a bridge among them.

3. **Switch nodes:** Switch nodes are a special kind of device nodes. They represent physical switches in the network which can be set to an *open* or *closed* configuration. Switch nodes govern how the electricity flows into the network. If a switch node is *open* then it NOT ALLOWS the flow of electricity through it. Only a *closed* switch ALLOWS the flow of electricity through it.
...
The dataset is available in `data` folder. There  are three CSV files with several fields of different types:
...
+ First, notice that all the nodes include all the properties for all of the three kinds of nodes indistinctly. The reason is that for simplicity we use homogeneous PGX graph format. There is also [heterogenous format]() where nodes can have different sets of properties.

+ Second, notice that the JSON file specifies the format of the file to be *edge_list*. The edge list format is a plain text format used by PGX to represent graphs. Please read about [plain text formats](reference/loader/file-system/plain-text-formats.html) to better understand the internal details of edge lists.
...
BFS is an algorithm for traversing graphs. It starts at some arbitrary node sometimes referred to as *source node* and explores its neighbour nodes first, then it moves to the next level of neighbours and repeats the process. However, we are interested in modifying BFS to obey the additional restriction of not trespassing those nodes that represent *open* switches. To achieve this, we wrote a custom Green-Marl program. Green-Marl is a domain specific language especially tailored to write graph related programs. It naturally takes advantage of the available parallel execution capabilities of PGX's architecture.
...
+ Create two properties, `bfs_parent` and `bfs_level`, that serve as containers for the output of the custom BFS Green-Marl program defined before.
...
+ Once the traversal has executed over the graph, those reached nodes should have their `bfs_level` property set to a number equal or greater than 0. To retrieve the traversed nodes, the PGX script perform the following PGQL query:
...
When the script terminates, you should see an output similar to what is below.
...
As the results show, our modified BFS algorithm traversed 10911 nodes. We can modify the configuration of some switch nodes to change the result. Let `s1` and `s2` be the switch nodes corresponding to the nodes with id numbers *8589937331* and *8589937325* respectively. Both of these nodes have their `switch_default` property set to `true`, by changing it to `false` they will not allow the flow of energy through them in the simulation. Add the following lines `getReachablePGQL.groovy` just beneath the source selection instruction `s = g.getVertex(21474836490)` in line 15 and save the changes.
...
To finish the use case, run again the `getReachablePGQL.groovy` script and look at the results below. As you may notice the restriction made to the switch nodes `sw1` and `sw2` prevented the BFS traversal from passing through them, thus reducing the number of nodes reachable from the source.
...
PGX is a fast, parallel, in-memory graph analytics framework. This parallelisation capability allows it to operate efficiently over large graphs without noticeable loss of performance.

To test the scalability of PGX, we can scale up the original electrical network model and repeat the custom BFS traversal over this new graph. This scaling can be done by recreating the original structure of the electrical network graph constructing several unconnected components. For this example, we are going to scale the model to a model 100 times bigger. The figure below exposes the idea of the BFS algorithm running on the scaled model. Notice how the number of reached nodes does not increase since the BFS is confined to a single component.
...
The script reads `electric_graph.edge` and saves 100 nodes for each of the nodes in it. This is done by adding an offset value to each of the node identification numbers for every time the original node is stored multiple times. The process is repeated for each of the edges in the file as well. The final result is saved to `electric_graph100x.edge`.
...
Notice how the BFS execution time barely doubled although the graph in memory was 100 times larger than the original. You can repeat what you learned on this section to perform the same experiment over graphs even bigger, but for your convenience, we have performed it over graphs 12, 100, 150, 500, 1000, 2000 and 10000 times larger. The results can be seen in the next table.
